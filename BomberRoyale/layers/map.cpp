...
bool Map::loadFromFile(const std::string& filename)
{
    ...
    //Create a string for reader
    std::string str;

    // Read data from file into root objects
    bool parsingSuccessful = Json::parseFromStream(reader, file, &root, &str);
    ...
    {
        if (layer["name"].asString() != "objects" && layer["name"].asString() != "walls")
            loadTileLayer(layer);
        else
            loadObjectLayer(layer, layer["name"].asString() == "objects");
    }
...
void Map::loadObjectLayer(Json::Value& layer, bool LoadInToObjects)
{
